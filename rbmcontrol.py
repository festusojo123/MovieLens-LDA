#lines 6-201 follows same procedure from 'recommender.py' but stores output in ldadict in line 204 to compare to control data from Microsoft's implementation of the RBM 
# algorithm on the same input data (MovieLens 1M) on the same user found at https://github.com/microsoft/recommenders/blob/master/examples/00_quick_start/rbm_movielens.ipynb
#finds recommendations both algorithms output for user and stores in commonDict in line 214
#calculates similarity value between our implementation and Microsoft's RBM from lines 218-226
...
def get_recommendations(user, sim_mat, ratings_mat, k, n, threshold_rating=3):
	...
	return list(sorted_L.keys())[:min_k]
...
f = open('rbm100.csv','r') #rbm100.csv contains the stored recommendations from Microsoft's RBM implementation
reader = csv.reader(f)
